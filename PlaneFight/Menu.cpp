...
void Menu::mouseMoveEvent(QMouseEvent* _event) {
	int _x = _event->x(), _y = _event->y();
	if (_x > startButtons[0]->geometry().left() && _x < startButtons[0]->geometry().right() &&
	    stackWidget->currentIndex() == 0) {
		if (_y > startButtons[0]->geometry().top() && _y < startButtons[0]->geometry().bottom()) {
		...
		} else if (_y > startButtons[1]->geometry().top() &&
		           _y < startButtons[1]->geometry().bottom()) {
			startLabels[0]->setPixmap(QPixmap("none"));
			startLabels[1]->setPixmap(QPixmap(QString::fromUtf8(":/PlaneFight/img/planeIcon.svg")));
			startLabels[2]->setPixmap(QPixmap("none"));
		} else if (_y > startButtons[2]->geometry().top() &&
		           _y < startButtons[2]->geometry().bottom()) {
			startLabels[0]->setPixmap(QPixmap("none"));
			startLabels[1]->setPixmap(QPixmap("none"));
	...
	if (_x > gameModes[0]->geometry().left() && _x < gameModes[0]->geometry().right() &&
	    stackWidget->currentIndex() == 1) {
		if (_y > gameModes[0]->geometry().top() && _y < gameModes[0]->geometry().bottom()) {
		...
		} else if (_y > gameModes[1]->geometry().top() && _y < gameModes[1]->geometry().bottom()) {
			modeLabels[0]->setPixmap(QPixmap("none"));
			modeLabels[1]->setPixmap(QPixmap(QString::fromUtf8(":/PlaneFight/img/planeIcon.svg")));
			modeLabels[2]->setPixmap(QPixmap("none"));
		} else if (_y > gameModes[2]->geometry().top() && _y < gameModes[2]->geometry().bottom()) {
			modeLabels[0]->setPixmap(QPixmap("none"));
			modeLabels[1]->setPixmap(QPixmap("none"));
